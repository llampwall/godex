...
export function NewRepoModal({ open, onOpenChange }: NewRepoModalProps) {
  ...
  const isValid = repoName.trim() && (useAIDescription ? aiDescription.trim() : selectedTemplate)

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] bg-card border-border p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4">
          <DialogTitle className="text-xl font-semibold text-foreground">Create a new repository</DialogTitle>
            ...
            {!useAIDescription ? (
              ...
              <div className="space-y-2">
                <Textarea
                  ...
                  className="min-h-[140px] bg-input border-border text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-ring resize-none"
                />
